...
- **模式开关**（颜色 / 语言等互斥模式）：一律用 **胶囊切换**（`.dp-segment`，§5.1.2 / [segment.md](segment.md)）。颜色默认 **`system`**，三项为跟随系统 / 浅色 / 深色。禁止单钮循环、矩形 tab 条、「深/浅」单字  
...
- [ ] 主题 / 语言等模式切换用胶囊；颜色默认跟随系统；无单钮循环 / 矩形 tab / 「深/浅」单字  
- [ ] 顶栏 64 · Logo 28–32 · 菜单/入口 14；功能入口无框；模式切换用胶囊  
