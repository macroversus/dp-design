...
**切换（D4-A · 2026-08-16）**  
默认 **`system`**（`prefers-color-scheme`）。顶栏**单钮循环**：跟随系统 → 浅色 → 深色 → 跟随系统。图标表示**当前偏好**（显示器 / `time-sun` / `time-moon`），不要三钮并列。持久化：`localStorage['dp-theme']` = `system` \| `light` \| `dark`。无存档时按 `system`。首屏用内联脚本解析，避免闪白。
...
| **模式类开关**（浅色/深色/跟随系统） | **单钮循环** 系统 → 浅 → 深；Secondary 边框 **32×32**；图标随当前偏好（显示器 / `time-sun` / `time-moon`）；须 `aria-label`；**禁止**「深/浅」单字 |
...
| 详情用面包屑回列表 | 只靠浏览器后退 |
| 流程用 Steps | 用侧栏假装步骤 |
| 主题单钮循环（系统 / 日 / 月） | 「深」「浅」单字按钮 / 三钮并列当默认 |
...
- [ ] 顶栏 64 · Logo 28–32 · 菜单/入口 14px；功能入口无框、模式开关有边框；主题单钮循环 系统→浅→深  
