...
**品牌与语义（D6-A）**  
Dark 下 Action / Primary 提亮为 `#a5adff`（Hover `#c0c5ff`；图标激活可用 `#d2d6ff`）；语义色保持色相并略提亮；Warning BG 等改为深底。浅底徽章上的字/图标用近白，避免深蓝叠深蓝。Product Dark 仍克制营销紫蓝。
...
- [ ] Dark 下 Action 提亮（约 `#a5adff`），选中字/图标可读；阴影为中性深阴影  
