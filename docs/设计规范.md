...
- **主色为公司蓝**（非青霓虹、非墨绿）：Primary / Action = `#165dff`（Hover `#4080ff` / Active `#0e42d2`）。  
- 展示站允许少量紫蓝辅助与渐变；工具站以中性灰阶 + 单一主蓝为主，克制装饰。  
- 字体以 **PingFang SC** 中文 + **Nunito Sans** 西文为主；内容强调可用 Merriweather。
...
自公司 DESIGN.md / Figma Color（Arco primary 色阶）提取，2026-08-18 起作为主色：

| Token（建议名） | 色值 | 来源含义 | 用法 |
|-----------------|------|----------|------|
| `--dp-color-primary` | `#165dff` | primary-6 | 主品牌蓝 / 强调 |
| `--dp-color-action` | `#165dff` | 同 Primary | 主按钮、链接、选中 |
| `--dp-color-primary-hover` | `#4080ff` | primary-5 | Hover |
| `--dp-color-primary-active` | `#0e42d2` | primary-7 | Active / Pressed |
| `--dp-color-primary-bg` | `#e8f3ff` | primary-1 | 浅蓝底 |
| `--dp-color-primary-bg-hover` | `#bedaff` | primary-2 | 浅蓝 Hover 底 |
| `--dp-color-action-bg-light` | `#e8f3ff` | primary-1 | 轻行动底 / Tag |
| `--dp-color-action-bg-hover` | `#bedaff` | primary-2 | 轻行动 Hover |
...
> 工具站**不要**把紫蓝当主色大面积使用；主行动统一落到 `#165dff`。
...
| `--dp-info` | `#165dff` | 信息（与主色同源） |

### 2.4 色彩使用规则

1. **一个页面只保留一个主行动色**（`#165dff`）。  
...
| 角色 | 推荐栈 | 依据 |
|------|--------|------|
| 中文 UI 正文 | `"PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", "Noto Sans SC", sans-serif` | 公司规范 + 两站最高频 |
| 西文 UI / 展示 | `"Nunito Sans", "PingFang SC", sans-serif` | 公司 Figma Fonts（Arco） |
...
| `--dp-radius-pill` | `100px` | 营销 CTA、少量 Chip、胶囊切换 |
...
**例外**  
- 子级为 **pill**（Chip / 开关 / 胶囊切换）：不套用公式，保持 pill。  
...
| Product 内容宽 | **无上限**：左右顶满工作区（侧栏后剩余空间），随窗口变宽 | `--dp-container` = `100%`；`--dp-container-max` = `none` |
...
| 模式 | 适用 | 结构 |
|------|------|------|
| **倒 L（Product 默认）** | Bohrium / fermopt / 报价内页 / Macroflow 壳层 | 侧栏通高 220（Logo 在窗口左上角）+ 右侧顶栏 64 + 画布 |
| **上下（Showcase）** | dp.tech / SynAsterHub / 报价首页 | 顶栏通栏（Logo 在最左 = 窗口左上角）+ 内容（最大 1280） |
| 左右（无顶栏） | 仅特殊全屏工具 | 侧栏 + 内容（非默认）；Logo 仍在侧栏顶 / 窗口左上角 |

**Logo 位置（硬规则）**  
不论 Product / Showcase，Logo **始终在整个窗口左上角**。  
Product：侧栏 **通高**，Logo 在侧栏顶区，与窗口顶边对齐；顶栏只占侧栏右侧，**禁止**顶栏通栏把 Logo 压到顶栏下方。  
Showcase：无侧栏，Logo 在顶栏最左侧。
...
5. **Product 不锁页宽**：内容区左右顶满工作区，随网页宽度动态铺开；禁止再套 `max-width: 1200`。表单字段区仍可 720 左对齐。Showcase 通栏仍 ≤1280。
...
| 激活 / 选中 | Action `#165dff`（`--dp-icon-color-active`） |
...
**定位**  
默认 **跟随系统**（`system`）；解析结果写入 `data-dp-theme="light|dark"` **重映射 CSS 变量**，禁止整页 `filter: invert`。

**适用范围（D2-A）**  
Product 工具站优先提供切换（工作台 / fermopt / 报价内页 / Macroflow 壳层），默认跟随系统。Showcase 落地页若无主题开关可锁 Light；有开关则同样默认 `system`。可保留局部深色 Hero（`--dp-bg-hero-dark`）；Hero 深色区块 ≠ 完整 Dark 主题。

**切换（D4-A · 2026-08-16）**  
默认 **`system`**（`prefers-color-scheme`）。颜色 / 语言等**模式切换一律用 §5.1.2 胶囊**（`.dp-segment`）：跟随系统 / 浅色 / 深色。禁止单钮循环、禁止矩形 tab 条、禁止「深/浅」单字。持久化：`localStorage['dp-theme']` = `system` \| `light` \| `dark`。无存档时按 `system`。首屏用内联脚本解析，避免闪白。
...
| `--dp-text-1…4` | 深灰阶 | **浅色**：`#f5f6f8` / `#d0d3e0` / `#a0a6bb` / `#9aa0b5` |
| `--dp-color-action-fg` | 同 Action | `#d2d6ff`（深色底上的链接/选中/图标） |

**Dark 文字硬规则（深色底上必须浅色）**

| 场景 | 必须用 | 禁止 |
|------|--------|------|
| 正文 / 标题 / 表格 / 面包屑 | `--dp-text-1/2/3` | 写死 `#1d2129` `#4e5969` `#020c1a` 等 Light 深色 |
| 辅助最弱档 | `--dp-text-4`（Dark 仍为浅灰 `#9aa0b5`） | `#5c637a` 及更深 |
| 链接 / 侧栏选中 / Tabs / 线型图标 | `--dp-color-action-fg`（`#bedaff`） | `--dp-color-action` 填充色 `#4080ff` 当字色 |
| 实心主按钮 / 实心色块字 | `--dp-text-on-primary` **白** | 深蓝/黑字叠在 Action 底上 |
| 浅底徽章（action-bg-light） | 近白或 `action-fg` | 深蓝字叠深蓝浅底 |

例外：Tooltip 等**浅色浮层**上可用深字；此时底必须是浅的，不是页面深底。
...
**品牌与语义（D6-A）**  
Dark 下实心 Action / Primary 填充为 `#4080ff`（Hover `#6aa1ff`），字 **白** `--dp-text-on-primary`；深色底上的链接/选中/图标字色用 `--dp-color-action-fg` / `--dp-icon-color-active`（`#bedaff`），**禁止**用填充色 `#4080ff` 当正文。浅底徽章用近白字。Product Dark 仍克制营销紫蓝。
...
**已覆盖：** 基础组件 + 胶囊切换（§5.1.2）+ §5.13–5.15（B1–B3）。  
**后续：** 业务特化组件按项目增量补充。
...
| 项 | 值 |
|----|-----|
| 高度 | **大 36 / 中 32（默认）/ 小 28 / 迷你 24**（`--dp-btn-height-*`） |
| 水平内边距 | 大 20 · 中/小 16 · 迷你 12 |
| 最小宽 | 大 117 · 中/小 109 · 迷你 90 |
| 字号 | 大/中/小 **14/CN-Regular**；迷你 **12/CN-Regular** |
| 圆角 | Product 控件 **8**（不采用公司稿 2px）；容器 16；Showcase CTA 可 pill |
| 图标按钮 | 36×36，图标 16；与文字间距 8 |
| 工具类图标钮 | 主题 / 下载 / 刷新等**优先图标**（见下），须 `aria-label` / `title`；**禁止**单字「深」「浅」代替昼夜 |
| Showcase 营销 CTA | 可选 40–48，不属于上述四档 |
...
| 底 | 字 / 字母色 |
|----|-------------|
| Primary / Action **实心填充**（含 Avatar 色块、Logo 色块、**胶囊切换选中段**） | **必须** `--dp-text-on-primary`（浅色）；禁止 Text 1 / 黑字 |
...
#### 5.1.2 胶囊切换（Segmented Control · 已锁定 2026-08-16）

> 预览：侧栏「组件样式」。类名 `.dp-segment`。实现细节见 Agent Pack **`segment.md`**。  
> 同一控件内互斥的 **2–4** 个模式（如「训练 + 预测 / 仅预测」）。**不是** Tabs（对象下的并列视图）、**不是** Switch（开/关）、**不是** 一排独立 Primary/Secondary。

| 项 | 值 |
|----|-----|
| 轨道 | 胶囊 `--dp-radius-pill`；底 `--dp-fill-2`；1px `--dp-border`；内边距 **3px**；`position: relative` |
| **Thumb** | `.dp-segment__thumb` 绝对定位；**唯一**承担选中 Action 底；`translate3d` + `width` 滑动 |
| 高度 | Product **36**（紧凑 32）；分段高 = 轨道 − 6 |
| 分段 | 胶囊；左右 padding **12–16**；字 **13–14 · 600**；**背景始终透明** |
| **选中** | Thumb = Action 高亮底；字 `--dp-text-on-primary`；按钮不加底 |
| 未选中 | 透明底；字 `--dp-text-1`（Dark 仍用 text-1，Token 已浅） |
| Hover（未选） | 轻底 `--dp-fill-3`；无位移 |
| 动效 | **240ms** `--dp-transition-medium`（已含 emphasize）；勿再叠一份 easing |
| Focus | 分段 Focus 环（Action 边 + 3px） |
| 数量 | 建议 2–4；超出改用 Select |
| 无障碍 | `role="radiogroup"`；项 `role="radio"` + `aria-checked`；thumb `aria-hidden` |
| 圆角 | 整组 pill，不套同心公式 |
...
| ✅ | ❌ |
|----|----|
| 独立 thumb 滑动承担选中底 | 给 `.is-active` 再铺 Action 底（双层闪烁） |
| 选中字 `--dp-text-on-primary` | 选中：白底描边 + 深蓝字 / 把 Action 当字色 |
| 自研 `.dp-segment` | 默认套 Ant Design `Segmented`（双层底 + 字色不受控） |
| Dark 选中仍是较深 Action + 白字 | Dark 选中改成浅底深字 / 仅描边高亮 |
...
**胶囊切换：** 互斥模式用 §5.1.2；独立 thumb 滑动 + 白字，不用白底描边、不给选中按钮铺底。  
...
| 顶区品牌 | Logo 在**窗口左上角**：侧栏通高，品牌区贴窗口顶边；图形高 **28px**；产品名 **14–16px · 600 · Text 1**；Logo↔名间距 **8px**；区内边距 **16–20px** |
...
| 区域 | Product | Showcase |
|------|---------|----------|
| 左 | 折叠 + 面包屑（或页标题）；**无 Logo**（Logo 在通高侧栏左上角） | Logo + 一级菜单 |
| 中 | 可选全局搜索 | — |
| 右 | 主题 · 通知 · 用户 | 登录 / 主 CTA |
...
禁止：顶栏先通栏再在下方侧栏放 Logo（Logo 会落到顶栏下面，不在窗口左上角）。
...
| **模式类开关**（颜色 / 语言等互斥模式） | **胶囊切换**（§5.1.2）；颜色三项：跟随系统 / 浅色 / 深色，默认可跟系统；须 `aria-label`；**禁止**单钮循环、矩形 tab、「深/浅」单字 |
...
| 组件 | 用法 |
|------|------|
| Tabs | 同一对象下的并列视图；**不作**站级主导航；**不作**表单内模式切换（模式用 §5.1.2 胶囊） |
| Steps | 线性向导，建议 3–5 步 |
...
| Logo 在整个窗口左上角（倒 L / 顶栏最左） | 顶栏通栏、Logo 在侧栏顶落到顶栏下方 |
...
| 详情用面包屑回列表 | 只靠浏览器后退 |
| 流程用 Steps | 用侧栏假装步骤 |
| 主题 / 语言用胶囊切换 | 「深」「浅」单字钮 / 单钮循环 / 矩形 tab 条 |
...
| 项 | 规则 |
|----|------|
| 壳层 | Product 倒 L（侧栏通高 + 右侧顶栏 64；Logo 窗口左上角） |
...
| 项 | 规则 |
|----|------|
| 默认布局 | 单栏；标签在上；表单内容最大宽 **720px**（Product 工作区内左对齐，页本身不锁 1200） |
...
| 阴影 | 可用蓝味阴影 | 轻阴影或边框 |
| 字体 | Nunito Sans + PingFang | Nunito Sans + PingFang |
...
[data-dp-theme="light"] {
  ...
  --dp-color-primary-bg: #e8f3ff;
  --dp-color-primary-bg-hover: #bedaff;
  ...
  --dp-shadow-emphasis: 0 4px 10px #9fbadc33, 0 6px 10px #165dff26;

  /* Typography */
  --dp-font-sans: "Nunito Sans", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", sans-serif;
  --dp-font-display: "Nunito Sans", var(--dp-font-sans);
...
- [ ] 默认主题跟随系统（`system`）；Dark 为解析结果（`data-dp-theme`），禁止 invert  
- [ ] Dark 下顶栏/侧栏/画布同步变深，无写死白底壳层  
- [ ] Logo 在整个窗口左上角（Product 倒 L / Showcase 顶栏左）  
- [ ] 顶栏 64 · Logo 28–32 · 菜单/入口 14px；功能入口无框；模式切换用胶囊（颜色 / 语言）  
...
- [ ] 胶囊切换选中 = Action 高亮底 + 白字；禁止白底描边当选中  
...
- [ ] Dark 下深色底文字全为浅色：text-1/2/3/4；选中/链接 `action-fg`；主按钮深 Action + 白字；无写死 Light 深色  
- [ ] Primary / CTA 为 `#165dff`（Light），Hover `#4080ff`，无青霓虹主色  
...
> 配套：[`CHANGELOG.md`](./CHANGELOG.md) · Agent Pack：`skills/dp-design/`（执行指令 `SKILL.md`）。当前版本：**0.1.17**（与业务站版本解耦）。
...
| **Agent Pack** | `skills/dp-design/` · 指南站介绍页下载 zip | 执行指令 `SKILL.md`（按需打开参考文件）；`AGENT.md` 为兼容入口；**不含**站点落地顺序 |
...
| Skill | 改站方从指南站介绍页下载 Agent Pack；以 `SKILL.md` 为准，按需打开参考文件 |

### 13.9 正反例

| ✅ | ❌ |
|----|----|
| 改主色 → 改 Token → 同步预览 CSS → Changelog | 只在某一业务站改 `#165dff` |
