...
| 链接 / 侧栏选中 / Tabs / 线型图标 | `--dp-color-action-fg`（`#d2d6ff`） | `--dp-color-action` 填充色 `#6b74f2` 当字色 |
...
**品牌与语义（D6-A）**  
Dark 下实心 Action / Primary 填充为 `#6b74f2`（Hover `#858df5`），字 **白** `--dp-text-on-primary`；深色底上的链接/选中/图标字色用 `--dp-color-action-fg` / `--dp-icon-color-active`（`#d2d6ff`），**禁止**用填充色 `#6b74f2` 当正文。浅底徽章用近白字。Product Dark 仍克制营销紫蓝。
