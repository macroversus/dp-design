...
> 预览：侧栏「组件样式」。类名 `.dp-segment`。  
> 同一控件内互斥的 **2–4** 个模式（如「训练 + 预测 / 仅预测」）。**不是** Tabs（对象下的并列视图）、**不是** Switch（开/关）、**不是** 一排独立 Primary/Secondary。

| 项 | 值 |
|----|-----|
| 轨道 | 胶囊 `--dp-radius-pill`；底 `--dp-fill-2`；1px `--dp-border`；内边距 **3px** |
| 高度 | Product **36**（紧凑 32）；分段高 = 轨道 − 6 |
| 分段 | 胶囊；左右 padding **12–16**；字 **13–14 · 600** |
| **选中** | **Action 实心高亮底** + **`--dp-text-on-primary` 浅色字** |
| 未选中 | 透明底；字 `--dp-text-1`（Dark 用 `--dp-text-2`） |
| Hover（未选） | 轻底 `--dp-fill-3`；无位移 |
| Focus | 分段 Focus 环（Action 边 + 3px） |
| 数量 | 建议 2–4；超出改用 Select |
| 无障碍 | `role="radiogroup"`；项 `role="radio"` + `aria-checked` |
| 圆角 | 整组 pill，不套同心公式 |
...
| ✅ | ❌ |
|----|----|
| 选中：`--dp-color-action` 高亮底 + 白字 | 选中：白底 / 浅底 + Action 描边 + 深蓝字 |
| 选中字 `--dp-text-on-primary` | 选中字用 Text 1，或把 Action 填充色当字色 |
...
**胶囊切换：** 互斥模式用 §5.1.2；选中 = Action 实心底 + 白字，不用白底描边。  
