...
| `--dp-radius` | `8px` | **Product 默认**（按钮、常规卡片） |
| `--dp-radius-lg` | `12px` | 中卡片、面板 |
| `--dp-radius-xl` | `16px` | 大卡 / Drawer / Modal |
| `--dp-radius-2xl` | `20px` | Showcase 大容器上限 |
| `--dp-radius-pill` | `100px` | 营销 CTA、少量 Chip（Showcase） |

**使用规则（S2-A）**  
Product 默认 **8px**；Showcase 大卡 **16–20px**；禁止全站统一超大 pill。
...
**品牌与语义（D6-A）**  
Dark 下 Action / Primary 提亮为 `#6b74f2`（Hover `#858df5`）；语义色保持色相并略提亮；Warning BG 等改为深底。Product Dark 仍克制营销紫蓝。
...
| 圆角 | Product 8；Showcase CTA 可 pill |
...
### 5.4 导航（已锁定 2026-08-11）

> 决策：N1–N8 全部采纳建议默认值。预览页：侧栏「导航」。  
> 尺寸沿用布局锁定：顶栏 64 · 侧栏 220/72。
...
| 文案 | 简洁名词；避免长句 |
| 图标 | 一级建议 20px 线型；折叠态仅图标 + Tooltip |
| 默认字色 | Text 2 `--dp-text-2` |
| Hover | 底 `--dp-fill-1`，字 Text 1；无位移 |
| 激活（N2-A） | 底 `--dp-color-action-bg-light` + 字 Action；**无左侧竖条** |
...
高度 64px；背景 `--dp-bg-header`（须跟主题，禁写死白）。
...
| **模式类开关**（浅色/深色主题等） | **按钮样式**：保留 Secondary 边框盒（仅图标亦可）；用 `time-sun` / `time-moon`；须 `aria-label`；**禁止**「深/浅」单字 |
| 用户 Avatar | 见 §5.14；实心 Action 底必须 on-primary 浅色字 |
...
| 顶栏功能入口无框（会话/智能体/对话/工作流） | 功能入口做成带边框按钮盒 |
| 主题等**模式开关**保留按钮边框 | 把主题钮也做成无框文字/抹掉边框 |
| 详情用面包屑回列表 | 只靠浏览器后退 |
| 流程用 Steps | 用侧栏假装步骤 |
| 主题切换用日月图标 | 「深」「浅」单字按钮 |
...
| 主色 | 品牌蓝 + 可选紫蓝渐变 | 单一行动蓝 |
| 圆角 | 12–20px 大卡，CTA 可 pill | 默认 8px |
| 阴影 | 可用蓝味阴影 | 轻阴影或边框 |
...
| 报价内页 / Macroflow 壳层 | Product | 灰底白卡、8px 圆角、蓝色主 CTA；Macroflow 取色环仍可保留节点色 |
...
- [ ] 顶栏同排菜单/工具钮：14px · Text 1 · 字重一致；主题用 sun/moon 图标非「深/浅」  
